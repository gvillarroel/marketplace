...
import {
  ...
  maximumVisibleOpenCodeRosterMembers,
...
type FilterField = "member" | "kind" | "status" | "capability" | "tool" | "skill" | "model" | "task" | "run" | "mode";
...
function runMatches(run: OpenCodeTeamRunSnapshot, filter: string): boolean {
  ...
  if (query.field === "task") return includes([run.task], query.value);
...
function formatUsage(
  usage: OpenCodeObservedUsage,
  turns: number | undefined,
  scope: OpenCodeTeamRunSnapshot["usageScope"],
  lowerBound: boolean,
  turnsLowerBound: boolean,
): string {
...
function availabilitySymbol(member: OpenCodeTeamMember): string {
...
function compactActivityLine(run: OpenCodeTeamRunSnapshot): string {
...
function boundView(lines: readonly string[]): string {
...
export function formatOpenCodeTeamView(snapshot: OpenCodeTeamSnapshot, filterInput = ""): string {
  ...
  const members = snapshot.members.filter((entry) =>
    memberMatches(entry, filter, activity.get(entry.id) ?? entry.availability));
...
export function formatOpenCodeTeamHelp(): string {
  return boundView([
...
export function formatOpenCodeStopResult(result: OpenCodeTeamStopResult): string {
  const lines = [
