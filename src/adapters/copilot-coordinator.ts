    ...
    await locked(() => {
      ...
      if (item) {
        ...
        if (outcome === "completed") {
          ...
          const error = (input as PostToolFailureHookInput).error ?? item.error ?? "Copilot task failed";
    ...
    onPreToolUse: async (input, invocation) => {
      ...
      return locked(async () => {
        try {
          ...
          if (snapshot.current?.id !== copilotFixedAgentIds.get("team-lead")) return;
  ...
  return {
    ...
    observeEvent: (event) => {
      ...
      if (event.type === "tool.execution_start" && !event.agentId && event.data?.toolName === "task" && toolCallId) {
        ...
        if (uncorrelated.length === 1) uncorrelated[0][1].invocationId = toolCallId;
