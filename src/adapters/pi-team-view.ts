...
import {
  ...
  type AgentHarborActivityOwnerRuntime,
...
function clipPiTeamLines(lines: readonly string[]): string[] {
  if (lines.length <= maximumPiTeamOverviewLines) return [...lines];
  ...
  while (prefix.length + preserved.length >= maximumPiTeamOverviewLines) prefix.pop();
...
type PiTeamPageKind = "roster" | "activity" | "history";

interface PiTeamPageRequest {
  readonly kind: PiTeamPageKind;
  readonly page: number;
}

type PiTeamViewRunSnapshot = PiTeamRunSnapshot & {
  readonly sharedOwnerRuntime?: Exclude<AgentHarborActivityOwnerRuntime, "opencode">;
  readonly sharedOwnerProcessID?: number;
};

interface SharedPiActivity {
  readonly runs: readonly PiTeamViewRunSnapshot[];
...
const piPagePrefixes = new Map<string, PiTeamPageKind>([
  ["roster-page", "roster"],
  ["activity-page", "activity"],
  ["history-page", "history"],
]);

function parsePiTeamPageRequest(filter: string, title: PiTeamViewOptions["title"]): PiTeamPageRequest | undefined {
...
function validatePiTeamFilter(filter: string, title: PiTeamViewOptions["title"]): void {
  if (!filter) return;
  const separator = filter.indexOf(":");
  ...
  if (!piTeamFilterFields.has(prefix)) {
    throw new Error(
      `unsupported /team field “${publicMetadataText(prefix, 40) ?? "empty"}”. ` +
...
function activityMatches(run: PiTeamViewRunSnapshot, filter: string): boolean {
  if (!filter) return true;
  const query = parsePiTeamFilter(filter);
  if (query.field === "status") {
  ...
  if (query.field === "heartbeat") return query.value === "overdue" && run.sharedHeartbeatOverdue === true;
...
function undisclosedTelemetryWarning(
  query: PiTeamFilter,
  runs: readonly PiTeamViewRunSnapshot[],
): string | undefined {
  if (query.field && !["model", "thinking", "task"].includes(query.field)) return undefined;
...
function sharedOwnerInstruction(run: PiTeamViewRunSnapshot): string {
...
function renderActivity(runs: readonly PiTeamViewRunSnapshot[], hasOtherActiveWork: boolean): string[] {
  if (!runs.length) return [hasOtherActiveWork ? "No active work matches this filter." : "No project-shared persistent-player work is visible; disposable contractor work is process-local."];
...
function compactPublicField(value: string, limit: number, fallback: string): string {
...
function compactRunLines(run: PiTeamViewRunSnapshot): string[] {
...
function sharedPiActivity(
  project: string,
  local: readonly PiTeamRunSnapshot[],
  ...
  try {
    ...
    const runs = claims
      ...
      .map((claim, index): PiTeamViewRunSnapshot => ({
...
function sharedActivityWarnings(shared: SharedPiActivity): string[] {
...
function emptyPiActivity(authoritative: boolean): string {
...
function renderCompactRuns(runs: readonly PiTeamViewRunSnapshot[], omittedLabel: string): string[] {
...
interface PiCapacitySummary {
  readonly localRootCount: number;
  readonly localPersistentRootCount: number;
  readonly localContractorRootCount: number;
  readonly projectSharedPersistentClaimCount?: number;
}

function renderCapacitySummary(summary: PiCapacitySummary): string[] {
...
function renderLeadAccess(
  ...
  capacitySummary: PiCapacitySummary,
...
function memberRepair(member: PiTeamMember): string | undefined {
...
function boundedPiTeamLine(value: string): string {
  const [bounded, remainder] = takeTerminalColumns(value, terminalLineWidth);
  if (!remainder) return bounded.trimEnd();
...
function renderBenchView(
  ...
  teamSummary: string,
  members: readonly PiTeamMember[],
  allMembers: readonly PiTeamMember[],
  activeMemberStates: ReadonlyMap<string, PiTeamRunSnapshot["state"]>,
  capacitySummary: PiCapacitySummary,
  activeCount: number,
  ...
  const maximumWithoutOmission = Math.max(0, Math.floor(
    (maximumPiTeamOverviewLines - prefix.length - suffix.length) / 2,
  ));
...
function pageBounds(total: number, requested: number, size: number): {
  readonly page: number;
  ...
  if (requested > pages) throw new Error(`page ${requested} is out of range; available pages: 1-${pages}`);
  const start = (requested - 1) * size;
  return { page: requested, pages, start, end: Math.min(total, start + size) };
}

function pageNavigation(kind: PiTeamPageKind, page: number, pages: number, bench: boolean): string {
  const route = (value: number) => bench
...
function renderPiHistoryPage(
  header: string,
  runtime: PiTeamRuntime,
  roots: readonly PiTeamRunSnapshot[],
  requestedPage: number,
  warnings: readonly string[] = [],
): string {
...
export async function formatPiTeamView(
  ...
  const unorderedActive: PiTeamViewRunSnapshot[] = [...localActive, ...shared.runs];
  ...
  const capacitySummary: PiCapacitySummary = {
  ...
  if (filter || options.title === "bench") return clipPiTeamLines(wrapped).join("\n");
  ...
  const compactOverview = (
    ...
    runLimit: number,
    factoryLimit = factoryMembers.length,
  ): string[] => {
