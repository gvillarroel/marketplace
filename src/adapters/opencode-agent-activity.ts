...
export type OpenCodeAgentActivityAdmissionValidation = (
  inventory: AgentHarborActivityAdmissionInventory,
) => void;

interface OpenCodeDestructiveRosterMutation {
...
type LegacyStoredClaim = Omit<StoredClaim, "version" | "ownerRuntime"> & { readonly version: 1 };
type ParsedStoredClaim = StoredClaim | LegacyStoredClaim;
...
interface ClaimPaths {
  ...
  readonly storeDirectory: ActivityStoreDirectory;
...
interface ReadCapacityLock {
  readonly value: StoredCapacityLock;
  readonly identity: FileIdentity;
  readonly mtimeNs: string;
  readonly ageMs: number;
}

interface ActivityCapacityLock {
  /** True only while this process still owns the exact published lock generation. */
  ownsExact(): boolean;
  release(): boolean;
}
...
type ActivityStoreDirectory = typeof activityDirectory | typeof sharedActivityDirectory;
...
const phaseCodes: Record<OpenCodeAgentActivityPhase, ParsedStoredClaim["phase"]> = {
  starting: "s",
  working: "w",
  cleaning: "c",
};
const phases: Record<ParsedStoredClaim["phase"], OpenCodeAgentActivityPhase> = {
  s: "starting",
  w: "working",
  c: "cleaning",
};
...
function claimPaths(
  project: string,
  create: boolean,
  storeDirectory: ActivityStoreDirectory = activityDirectory,
): ClaimPaths | undefined {
...
function parseClaim(value: unknown, expectedProject: string, expectedAgent: string): {
  readonly claim: ParsedStoredClaim;
  readonly sessionID: string;
} {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("invalid Agent Harbor shared activity claim");
  const legacyKeys = [
  ...
  if (![1, 2].includes(typeof claim.version === "number" ? claim.version : 0) ||
      ...
      !Number.isSafeInteger(claim.startedAt) || (claim.startedAt as number) < 0 ||
      !Number.isSafeInteger(claim.processID) || (claim.processID as number) <= 0 ||
  ...
  const stored = claim as unknown as ParsedStoredClaim;
...
function capacityLockPath(paths: ClaimPaths): string {
  return contained(paths.directory, join(paths.directory, capacityLockName));
}

function temporaryCapacityLockPath(paths: ClaimPaths, token: string): string {
  if (!tokenPattern.test(token)) throw new Error("invalid OpenCode capacity-lock publication token");
  return contained(paths.directory, join(paths.directory, `.agent-harbor-capacity-tmp-${token}`));
}

function parseCapacityLock(value: unknown, expectedProject: string): StoredCapacityLock {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("invalid OpenCode activity capacity lock");
  ...
  const lock = value as Partial<StoredCapacityLock>;
  ...
  return lock as StoredCapacityLock;
}

function readCapacityLock(paths: ClaimPaths, now = Date.now()): ReadCapacityLock | undefined {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    if (before.nlink !== 1n) throw new Error("OpenCode activity capacity-lock recovery is required");
    ...
    try {
      ...
      catch { throw new Error("invalid OpenCode activity capacity lock"); }
      const value = parseCapacityLock(parsed, paths.project);
      if (JSON.stringify(value) !== encoded) throw new Error("invalid OpenCode activity capacity lock");
      const ageMs = now - Number(before.mtimeMs);
      if (!Number.isFinite(ageMs) || ageMs < -5_000) throw new Error("invalid OpenCode activity capacity-lock timestamp");
...
function pauseCapacityContender(milliseconds: number): void {
  const shared = new Int32Array(new SharedArrayBuffer(4));
  Atomics.wait(shared, 0, 0, Math.max(1, milliseconds));
}

function acquireCapacityLock(paths: ClaimPaths): ActivityCapacityLock {
  const file = capacityLockPath(paths);
  ...
  for (;;) {
    ...
    if (existing) {
      ...
      if (existing.ageMs > openCodeCapacityLockTtlMs) {
        throw new Error(
          `Agent Harbor activity capacity lock is overdue but PID ${existing.value.processID} is live; ` +
      ...
      if (now >= deadlineAt) throw new Error("OpenCode activity capacity lock is busy; retry the command");
    ...
    try {
      ...
      const value: StoredCapacityLock = {
        version: 1,
        owner: "agent-harbor",
      ...
      return {
        ownsExact() {
          if (released || !lockIdentity) return false;
          try {
        ...
        release() {
          ...
          let current: ReadCapacityLock | undefined;
          try { current = readCapacityLock(paths); }
          catch { return false; }
          if (!current) { released = true; return true; }
          if (!lockIdentity || current.value.claimToken !== claimToken ||
              !sameIdentity(current.identity, lockIdentity) ||
              !removeOwnedPath(paths, file, lockIdentity, current.mtimeNs)) return false;
          released = true;
          return true;
        },
      };
      ...
      if (lockIdentity) {
        ...
        if (published) removeOwnedPath(paths, file, lockIdentity);
      }
...
function readClaim(paths: ClaimPaths, file: string, agent: string, now = Date.now()): ReadClaim | undefined {
  for (let attempt = 0; attempt < 3; attempt += 1) {
    ...
    try {
      ...
      let parsed: unknown;
      try { parsed = JSON.parse(encoded); }
      catch { throw new Error("invalid Agent Harbor shared activity claim"); }
      ...
      if (JSON.stringify(claim) !== encoded) throw new Error("invalid Agent Harbor shared activity claim");
      const age = now - Number(before.mtimeMs);
      if (!Number.isFinite(age) || age < -5_000) throw new Error("invalid Agent Harbor shared activity claim timestamp");
      return {
        snapshot: {
          agent: claim.agent,
          kind: claim.kind,
...
function removeOwnedPath(paths: ClaimPaths, file: string, expected: FileIdentity, expectedMtimeNs?: string): boolean {
  try {
    assertDirectoryBinding(paths);
    const current = safeClaimStat(file);
    if (!sameIdentity(identity(current), expected) ||
        expectedMtimeNs !== undefined && current.mtimeNs.toString(10) !== expectedMtimeNs) return false;
    unlinkSync(file);
    assertDirectoryBinding(paths);
    return true;
  } catch (error: any) {
...
function processDefinitelyExited(processID: number): boolean {
  try {
...
function removeExactClaim(
  ...
  try {
    ...
    if (!sameIdentity(current.identity, expected) || current.snapshot.claimToken !== claimToken) return false;
    return removeOwnedPath(paths, file, expected, current.mtimeNs);
  } catch { return false; }
}

function ownsExactClaim(
  paths: ClaimPaths,
  file: string,
  agent: string,
  expected: FileIdentity,
...
function removeDefinitelyExitedClaim(
  paths: ClaimPaths,
  file: string,
  agent: string,
  observed: ReadClaim,
  now: number,
  capacityLock: ActivityCapacityLock,
): boolean {
  if (!capacityLock.ownsExact()) return false;
  const confirmed = readClaim(paths, file, agent, now);
  if (!confirmed) return capacityLock.ownsExact();
  if (confirmed.fresh || !processDefinitelyExited(confirmed.snapshot.processID) ||
      ...
      !removeOwnedPath(paths, file, confirmed.identity, confirmed.mtimeNs)) return false;
  return capacityLock.ownsExact();
}

function projectClaimsWhileLocked(
  paths: ClaimPaths,
  includeStale: boolean,
  capacityLock: ActivityCapacityLock,
): readonly ReadClaim[] {
  if (!capacityLock.ownsExact()) throw new Error("OpenCode activity capacity lock ownership changed before inventory recovery");
  assertDirectoryBinding(paths);
  const inventory: Array<{ readonly agent: string; readonly file: string; readonly claim: ReadClaim }> = [];
  ...
  try {
    for (;;) {
      ...
      if (entry.name === capacityLockName) {
        ...
        if (!capacityLock.ownsExact()) throw new Error("OpenCode activity capacity lock ownership changed during inventory recovery");
        continue;
      }
      inspected += 1;
      ...
      if (claim) inventory.push({ agent: match[1], file, claim });
    }
  } finally {
    directory.closeSync();
  }
  assertDirectoryBinding(paths);
  if (!capacityLock.ownsExact()) throw new Error("OpenCode activity capacity lock ownership changed after inventory read");
  ...
  for (const item of inventory) {
    ...
    if (includeStale || claim.fresh || !processDefinitelyExited(claim.snapshot.processID)) claims.push(claim);
  }
  if (!capacityLock.ownsExact()) throw new Error("OpenCode activity capacity lock ownership changed after stale-claim recovery");
  return claims;
}
...
function projectClaims(
  ...
  includeStale: boolean,
  storeDirectory: ActivityStoreDirectory = activityDirectory,
  heldCapacityLock?: ActivityCapacityLock,
): readonly ReadClaim[] {
  const paths = claimPaths(project, false, storeDirectory);
  if (!paths) return [];
  if (heldCapacityLock) return projectClaimsWhileLocked(paths, includeStale, heldCapacityLock);
  ...
  if (existingLock && !(existingLock.ageMs > openCodeCapacityLockTtlMs &&
    ...
    if (existingLock.ageMs > openCodeCapacityLockTtlMs) {
      throw new Error(
        `Agent Harbor activity capacity lock is overdue but PID ${existingLock.value.processID} is live; ` +
        "possible PID reuse cannot be reclaimed safely. Stop the owning process, or after proving no Agent Harbor work is active remove only the stale managed lock and retry",
      );
    }
  ...
  let value: readonly ReadClaim[] | undefined;
  let failure: unknown;
  try { value = projectClaimsWhileLocked(paths, includeStale, capacityLock); }
  ...
  if (!released) throw new Error("OpenCode activity inventory recovered but capacity-lock cleanup could not be verified");
  return value!;
}
...
function writeAll(descriptor: number, bytes: Buffer, position = 0): void {
  ...
  while (offset < bytes.length) {
    ...
    if (written <= 0) throw new Error("Agent Harbor shared activity claim write made no progress");
    offset += written;
  }
}

function createOpenCodeAgentActivityClaim(
  project: string,
  agent: string,
  kind: OpenCodeAgentActivityKind,
  sessionID: string,
  ownerRuntime: AgentHarborActivityOwnerRuntime,
  now = Date.now(),
  validateAdmission?: OpenCodeAgentActivityAdmissionValidation,
  storeDirectory: ActivityStoreDirectory = activityDirectory,
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    const releaseCapacity = (): boolean => {
      if (capacityReleased) return true;
      const result = capacityLock.release();
      if (result) capacityReleased = true;
      return result;
    };
    let freshClaims: readonly ReadClaim[];
    try {
      ...
      if (existing) {
        ...
        if (!processDefinitelyExited(existing.snapshot.processID)) {
          throw new Error(
            `Agent Harbor player ${agent} has an overdue activity heartbeat but PID ${existing.snapshot.processID} is live; ` +
    ...
    if (kind === "direct" && freshClaims.some(({ snapshot }) =>
      snapshot.kind === "direct" && snapshot.sessionID === sessionID)) {
      if (!releaseCapacity()) throw new Error("OpenCode direct-session admission and capacity-lock cleanup both failed");
    ...
    if (freshClaims.length >= maximumOpenCodeAgentActivitiesPerProject) {
      if (!releaseCapacity()) throw new Error("OpenCode activity capacity-lock cleanup failed at the project limit");
    ...
    let activeSlot: ParsedStoredClaim["slot"] = "a";
    let released = false;
    let descriptor: number | undefined;
    let fileIdentity: FileIdentity | undefined;
    let heartbeat: ReturnType<typeof setInterval> | undefined;
    let ownershipLost = false;
    const ownershipLostListeners = new Set<() => void>();
    let published = false;
    try {
      ...
      if (encodedBytes.length > maximumOpenCodeActivityClaimBytes) throw new Error("Agent Harbor shared activity claim exceeds its safety bound");
      ...
      if (!releaseCapacity()) throw new Error("OpenCode activity capacity-lock cleanup could not be verified");

      const beat = (): void => {
        if (released || descriptor === undefined || !fileIdentity) return;
        const notifyOwnershipLost = (): void => {
          if (ownershipLost || released) return;
          ownershipLost = true;
          if (heartbeat) { clearInterval(heartbeat); heartbeat = undefined; }
          for (const listener of ownershipLostListeners) {
            try { listener(); } catch { /* Ownership loss remains authoritative even if an observer fails. */ }
          }
      ...
      return {
        ...
        setSessionID(next) {
          ...
          if (released || descriptor === undefined || !fileIdentity || kind !== "delegated" ||
              ...
              !ownsExactClaim(paths, file, agent, fileIdentity, claimToken)) return false;
          try {
            ...
            beat();
            return true;
          } catch { return false; /* The active slot still points to the last complete session identity. */ }
        },
        setPhase(next) {
          ...
          try {
            ...
            beat();
            return true;
          } catch { return false; /* Keep the last safely published phase and continue heartbeating the ownership claim. */ }
        },
        onOwnershipLost(listener) {
          if (typeof listener !== "function") throw new Error("invalid Agent Harbor activity ownership-loss listener");
          if (ownershipLost) {
            try { listener(); } catch { /* The loss state is already durable. */ }
            return () => undefined;
          }
        ...
        release() {
          ...
          if (fileIdentity && !removeExactClaim(paths, file, agent, fileIdentity, claimToken)) return false;
          released = true;
          ownershipLostListeners.clear();
          return true;
        },
...
export function claimValidatedOpenCodeAgentActivity(
  project: string,
  agent: string,
  kind: OpenCodeAgentActivityKind,
  sessionID: string,
  ...
  if (typeof validateAdmission !== "function") throw new Error("invalid OpenCode activity admission validation");
...
export function claimSharedAgentActivity(
  project: string,
  agent: string,
  kind: OpenCodeAgentActivityKind,
  runID: string,
  ownerRuntime: Exclude<AgentHarborActivityOwnerRuntime, "opencode">,
  now = Date.now(),
): OpenCodeAgentActivityClaim {
  return createOpenCodeAgentActivityClaim(
    project, agent, kind, runID, ownerRuntime, now, undefined, sharedActivityDirectory,
  );
}

/** Shared claim whose final roster/configuration validation runs under the cross-process gate. */
export function claimValidatedSharedAgentActivity(
  project: string,
  agent: string,
  kind: OpenCodeAgentActivityKind,
  runID: string,
  ...
  if (typeof validateAdmission !== "function") throw new Error("invalid shared activity admission validation");
  return createOpenCodeAgentActivityClaim(
    project, agent, kind, runID, ownerRuntime, now, validateAdmission, sharedActivityDirectory,
  );
}

function destructiveRosterMutation(
  command: CommandName,
  args: string,
): OpenCodeDestructiveRosterMutation | undefined {
  if (command === "retire") {
...
export function runOpenCodeRosterMutationGate<T>(
  command: CommandName,
  args: string,
  project: string,
  action: () => Promise<T>,
 ...
 */
export async function withOpenCodeRosterMutationGate<T>(
  project: string,
  ...
  return withRosterMutationGate(
    project, targets, actionLabel, action, activityDirectory, ignoredClaimToken,
  );
}

async function withRosterMutationGate<T>(
  project: string,
  targets: readonly string[],
  actionLabel: string,
  action: () => Promise<T>,
  storeDirectory: ActivityStoreDirectory,
  ignoredClaimToken?: string,
): Promise<T> {
  ...
  try {
    ...
    const wildcard = claims.find(({ snapshot }) =>
      snapshot.agent === "team-lead" || snapshot.agent === "talent-scout");
  ...
  if (failure !== undefined) throw failure;
  if (!released) throw new Error("OpenCode roster mutation succeeded but activity-gate cleanup could not be verified");
  return value!;
}

/** Shared Pi/Copilot destructive-mutation gate; an exact owner may mutate for its own scout run. */
export function withSharedRosterMutationGate<T>(
  project: string,
  targets: readonly string[],
  actionLabel: string,
  action: () => Promise<T>,
  ...
  return withRosterMutationGate(
    project, targets, actionLabel, action, sharedActivityDirectory, ignoredClaimToken,
  );
}
...
export function readOpenCodeAgentActivities(project: string): readonly OpenCodeAgentActivitySnapshot[] {
  ...
  return claims.map(({ snapshot, fresh }) => ({ ...snapshot, ...(fresh ? {} : { heartbeatOverdue: true as const }) }));
}

/** Private stop/recovery inventory; overdue claims remain observable unless exact dead-owner recovery succeeds. */
export function readOpenCodeAgentActivitiesIncludingStale(project: string): readonly OpenCodeAgentActivitySnapshot[] {
  return projectClaims(project, true)
    .map(({ snapshot, fresh }) => ({ ...snapshot, ...(fresh ? {} : { heartbeatOverdue: true as const }) }));
}

/** Bounded live claims shared across Pi and Copilot OS processes. */
export function readSharedAgentActivities(project: string): readonly OpenCodeAgentActivitySnapshot[] {
  ...
  return claims.map(({ snapshot, fresh }) => ({
    ...snapshot,
    ...(fresh ? {} : { heartbeatOverdue: true as const }),
  }));
}
