...
function benchMutationRows(raw: string): Array<{ readonly id: string; readonly action: "on" | "off" }> {
  return raw.split(/\r?\n/gu).flatMap((line) => {
    const match = /^([a-z0-9-]+): turned (on|off)$/u.exec(line.trim());
    return match ? [{ id: match[1], action: match[2] as "on" | "off" }] : [];
  });
}

function conciseDirectResult(api: TuiPluginApi, command: DeterministicCommandName, args: string, raw: string): string {
  if (command === "bench" && !args.trim().startsWith("list")) {
    const rows = benchMutationRows(raw);
    if (!rows.length) return boundedPublicMultiline(raw) ?? "The Agent Harbor bench action completed.";
...
export async function runOpenCodeTeamQuery(
  ...
  if (["help", "--help", "-h", "?"].includes(query.toLowerCase())) {
    if (isLatest()) showTeamDialog(api, "Agent Harbor team help · 0 model tokens", formatOpenCodeTeamHelp());
    return;
  }
  try {
    if (stopQuery) {
      ...
      if (selector.length > 256 || Buffer.byteLength(selector, "utf8") > 256) throw new Error("/team stop selector exceeds the 256-byte safety limit");
      ...
      showTeamDialog(
        api,
        `Agent Harbor team · ${result.failed.length ? "stop incomplete" : "stop complete"}`,
        `${formatOpenCodeStopResult(result)}\n\n${refreshMessage}`,
      );
      return;
    }
    ...
    if (isLatest()) showTeamDialog(api, "Agent Harbor team · 0 model tokens", formatOpenCodeTeamView(snapshot, query));
  } catch (error) {
    if (!isLatest()) {
    ...
    showTeamDialog(
      api,
      "Agent Harbor team · action unavailable",
      openCodePublicLabel(message(error), 600) ?? "The OpenCode team action failed without a public error message.",
    );
  }
}
...
export function openCodeDirectCommands(api: TuiPluginApi): DirectTuiCommand[] {
  const execute = async (command: DeterministicCommandName, args: string): Promise<void> => {
    ...
    try {
      ...
      if (mutating) snapshotReads.delete(api);
