...
import { parseContractDefinition, type HarborCommandResult } from "../core/commands.js";
...
import {
  ...
  type OpenCodeContractTelemetry,
  type OpenCodeOrchestratorClient,
} from "../orchestrators/opencode.js";
...
import {
  ...
  isOpenCodeAgentConfigured,
  isOpenCodeAgentLoaded,
  openCodePublicLabel,
  readOpenCodeDirectAliasCollisions,
  stopOpenCodeTeamRuns,
  type OpenCodeTeamRuntimeOptions,
...
function boundedTeamActionError(error: unknown, fallback: string): string {
  const safe = publicErrorText(message(error), 600) ?? fallback;
  const wrapped = wrapPlainLines([safe]);
  if (wrapped.length <= maximumOpenCodeTeamDialogLines) return wrapped.join("\n");
  return [
...
const displayEpoch = new WeakMap<TuiPluginApi, number>();
const disposedApis = new WeakSet<TuiPluginApi>();
const harborDialogState = new WeakMap<TuiPluginApi, { readonly baseSize: TuiPluginApi["ui"]["dialog"]["size"]; readonly token: object }>();
const mutationTails = new WeakMap<TuiPluginApi, Promise<void>>();
const snapshotReads = new WeakMap<TuiPluginApi, Promise<Awaited<ReturnType<typeof collectOpenCodeTeamSnapshot>>>>();

function v2TextParts(value: unknown): Array<{ type: "text"; text: string }> {
  if (!Array.isArray(value) || value.length !== 1) {
    throw new Error("OpenCode TUI contracts require exactly one text prompt part");
  }
  const part = value[0] as { readonly type?: unknown; readonly text?: unknown } | null;
...
export function openCodeTuiOrchestratorClient(
  client: TuiPluginApi["client"],
): OpenCodeOrchestratorClient {
  return {
    session: {
      async create(input) {
...
function formatContractTelemetry(telemetry: OpenCodeContractTelemetry): string {
...
function beginDisplayAction(api: TuiPluginApi): number {
  const epoch = (displayEpoch.get(api) ?? 0) + 1;
  displayEpoch.set(api, epoch);
  return epoch;
}

function isLatestDisplayAction(api: TuiPluginApi, epoch: number): boolean {
  return !isDisposed(api) && displayEpoch.get(api) === epoch;
}

function isDisposed(api: TuiPluginApi): boolean {
  return disposedApis.has(api) || api.lifecycle?.signal.aborted === true;
}

function enqueueMutation<T>(api: TuiPluginApi, action: () => Promise<T>): Promise<T> {
  if (mutationTails.has(api)) return Promise.reject(new Error("Another Agent Harbor mutation is already running; wait for its result before retrying"));
  snapshotReads.delete(api);
  const result = Promise.resolve().then(() => {
    if (isDisposed(api)) throw new Error("Agent Harbor TUI controls were deactivated");
    return action();
  });
...
function sharedTeamSnapshot(
  api: TuiPluginApi,
  options: OpenCodeTeamRuntimeOptions,
): Promise<Awaited<ReturnType<typeof collectOpenCodeTeamSnapshot>>> {
  ...
  void reading.finally(() => {
    if (snapshotReads.get(api) === reading) snapshotReads.delete(api);
  }).catch(() => undefined);
  return reading;
}

function prepareHarborDialog(api: TuiPluginApi) {
  ...
  const isCurrent = (): boolean => harborDialogState.get(api)?.token === token;
  const close = (): boolean => {
...
function clearOwnedHarborDialog(api: TuiPluginApi): void {
...
function showTeamDialog(api: TuiPluginApi, title: string, contents: string): void {
  if (isDisposed(api)) return;
...
function boundedPublicMultiline(value: string, maximumCodePoints = 24_000): string | undefined {
  ...
  const lines = sourceLines.slice(0, 256).map((line) => {
    if ([...line].length > 1_000) truncated = true;
  ...
  if (points.length > maximumCodePoints) truncated = true;
...
function boundedCatalogOutput(value: string): string {
...
export function boundedContractEvidence(value: string, maximumCodePoints = 24_000): string | undefined {
  const source = value.slice(0, Math.min(48_000, maximumCodePoints * 2));
  const sourceLines = source.split(/\r?\n/gu);
  let truncated = source.length < value.length || sourceLines.length > 256;
  const safeLines = sourceLines.slice(0, 256).map((line) => {
    ...
    const points = [...safe];
    if (points.length > 1_000) truncated = true;
    return points.slice(0, 1_000).join("");
  });
  const wrapped = wrapPlainLines(safeLines).filter((line, index, all) =>
    line.trim() || all.slice(0, index).some((candidate) => candidate.trim()) || all.slice(index + 1).some((candidate) => candidate.trim()));
  if (!wrapped.some((line) => line.trim())) return undefined;
  ...
  if (!truncated) return selected.join("\n").trim();
  const notice = `… evidence bounded; ${Math.max(0, wrapped.length - selected.length)} wrapped lines omitted before telemetry.`;
  if (selected.length >= maximumEvidenceLines) selected.splice(maximumEvidenceLines - 1);
  selected.push(notice);
  return selected.join("\n").trim();
}

function requireBoundedDirectInput(command: DeterministicCommandName, args: string): void {
...
function conciseDirectResult(
  ...
  if (command === "bench" && !isBenchListRequest(args)) {
    ...
    if (lifecycle?.command !== "bench") throw new Error("OpenCode bench lifecycle verification was not retained");
    ...
    if (rows.length > 4) {
      ...
      const lines = shown.map(({ id, action, status }) => {
        ...
        if (unavailableAlias) collisions.add(unavailableAlias);
  ...
  if (command === "retire") {
    const id = args.trim();
    const lifecycle = result.lifecycle;
    if (lifecycle?.command !== "retire") throw new Error("OpenCode retire lifecycle verification was not retained");
  ...
  if (command === "list-skills") return boundedCatalogOutput(raw);
  if (command !== "join") return boundedPublicMultiline(raw) ?? "The Agent Harbor action completed.";
  ...
  const lifecycle = result.lifecycle;
  if (lifecycle?.command !== "join") throw new Error("OpenCode join lifecycle verification was not retained");
...
export async function runOpenCodeTeamQuery(
  api: TuiPluginApi,
  input: string,
  options: OpenCodeTeamRuntimeOptions = {},
): Promise<void> {
  ...
  const isLatest = (): boolean => isLatestDisplayAction(api, epoch);
  ...
  if (["help", "--help", "-h", "?"].includes(query.toLowerCase())) {
    if (isLatest()) showTeamDialog(
      api,
      "Agent Harbor team help · 0 model tokens",
      formatOpenCodeTeamHelp(readOpenCodeDirectAliasCollisions(api.state.path.directory)),
    );
    return;
  }
  try {
    ...
    if (stopQuery) {
      ...
      if (!selector) throw new Error("usage inside the /team prompt: stop <run-id|all>");
      if (selector.length > 256 || Buffer.byteLength(selector, "utf8") > 256) throw new Error("the /team prompt stop selector exceeds the 256-byte safety limit");
      ...
      if (!isLatest()) {
        if (!isDisposed(api)) api.ui.toast({
      ...
      showTeamDialog(
        api,
        `Agent Harbor team · ${stopIncomplete ? "stop incomplete" : "stop complete"} · 0 model tokens`,
        formatOpenCodeStopResult(result, refreshMessage),
      );
      return;
    }
    ...
    if (isLatest()) showTeamDialog(
      api,
      diagnosticsMatch ? "Agent Harbor team diagnostics · 0 model tokens" : "Agent Harbor team · 0 model tokens",
      diagnosticsMatch
    ...
    if (!isLatest()) {
      if (stopQuery && !isDisposed(api)) api.ui.toast({
        variant: "error",
    ...
    showTeamDialog(
      api,
      "Agent Harbor team · action unavailable · 0 model tokens",
      boundedTeamActionError(error, "The OpenCode team action failed without a public error message."),
    );
  }
}
...
export function openCodeDirectCommands(api: TuiPluginApi): DirectTuiCommand[] {
  const execute = async (command: DeterministicCommandName, args: string): Promise<void> => {
    ...
    try {
      ...
      const invoke = () => runOpenCodeRosterMutationGate(
        command,
        args,
      ...
      const mutate = async (): Promise<HarborCommandResult> => {
        if (command === "retire") {
          const memberID = args.trim();
          const assertInactive = (snapshot: Awaited<ReturnType<typeof collectOpenCodeTeamSnapshot>>): void => {
      ...
      if (mutating && changed) snapshotReads.delete(api);
      if (!isLatestDisplayAction(api, epoch)) {
        if (mutating && !isDisposed(api)) api.ui.toast({
          variant: "success",
      ...
      if (persistent) showTeamDialog(api, "Agent Harbor · 0 model tokens", result);
      else api.ui.toast({ variant: "success", title: "Agent Harbor · 0 model tokens", message: result, duration: 10_000 });
    } catch (error) {
  ...
  const executeContract = async (args: string): Promise<void> => {
  ...
  return [
    metadata("team", "Agent Harbor: team status", "Show roster and active work, filter it, get help, or stop verified Harbor runs without a model request.", "team", () => {
      ...
      showHarborPrompt(
        api,
        "Agent Harbor team · 0 model tokens",
        "filter, help, or stop <run-id|all>; Enter shows all",
        (value) => runOpenCodeTeamQuery(api, value),
      );
    }),
