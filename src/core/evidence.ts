...
function validateEvidenceLimit(maximumBytes: number): void {
  if (!Number.isSafeInteger(maximumBytes) || maximumBytes < 256 ||
      maximumBytes > maximumConfigurableHarborEvidenceBytes) {
    throw new Error(
      `Agent Harbor evidence byte limit must be an integer between 256 and ${maximumConfigurableHarborEvidenceBytes}`,
    );
  }
}
...
export function boundHarborEvidence(
  value: string,
  maximumBytes = maximumHarborEvidenceBytes,
): BoundedHarborEvidence {
  validateEvidenceLimit(maximumBytes);
  const utf8Bytes = Buffer.byteLength(value, "utf8");
  if (utf8Bytes <= maximumBytes) return { text: value, utf8Bytes, truncated: false };
...
export class HarborEvidenceAccumulator {
  ...
  private omittedSegments = 0;

  constructor(private readonly maximumBytes = maximumHarborEvidenceBytes) {
    validateEvidenceLimit(maximumBytes);
  }
  ...
  markIncomplete(omittedSegments = 1): void {
