...
function validPathInput(value: string): void {
...
export function canonicalProjectIdentity(project: string): string {
  const canonical = canonicalPathIdentity(project);
  try {
    const stat = lstatSync(canonical);
    if (!stat.isDirectory()) throw new Error("Agent Harbor project path is not a directory");
  } catch (error: any) {
...
export function sameCanonicalProject(left: string, right: string): boolean {
