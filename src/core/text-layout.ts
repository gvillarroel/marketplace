...
function graphemeWidth(value: string): number {
  if (!value || onlyZeroWidthCharacters.test(value)) return 0;
  if (emojiGrapheme.test(value)) return 2;
...
function controlStringEnd(value: string, start: number, allowBell: boolean): number | undefined {
  for (let index = start; index < value.length; index += 1) {
    if (allowBell && value.charCodeAt(index) === 0x07) return index + 1;
...
function csiEnd(value: string, start: number): number | undefined {
  for (let index = start; index < value.length; index += 1) {
...
function shortEscapeEnd(value: string, start: number): number | undefined {
...
interface AnsiSequence {
  /** End of a complete sequence. */
  readonly end?: number;
  /** Bytes to discard when an introducer is incomplete, leaving its payload visible and wrappable. */
  readonly incompleteIntroducerEnd: number;
}

/** Identifies one ANSI/ECMA-48 control sequence without accepting an unterminated introducer. */
function ansiSequence(value: string, index: number): AnsiSequence | undefined {
  const code = value.charCodeAt(index);
  if (code === 0x1b) {
    const next = value.charCodeAt(index + 1);
    if (Number.isNaN(next)) return { incompleteIntroducerEnd: index + 1 };
    if (next === 0x5b) return { end: csiEnd(value, index + 2), incompleteIntroducerEnd: index + 2 }; // CSI: ESC [
    if (next === 0x5d) return { end: controlStringEnd(value, index + 2, true), incompleteIntroducerEnd: index + 2 }; // OSC: ESC ]
  ...
  if (code === 0x9b) return { end: csiEnd(value, index + 1), incompleteIntroducerEnd: index + 1 };
  if (code === 0x9d) return { end: controlStringEnd(value, index + 1, true), incompleteIntroducerEnd: index + 1 };
...
function terminalTokens(value: string): TerminalToken[] {
  ...
  const pushText = (text: string): void => {
    for (const { segment } of graphemes.segment(text)) tokens.push({ value: segment, width: graphemeWidth(segment) });
  };
  for (let index = 0; index < value.length;) {
    ...
    if (textStart < index) pushText(value.slice(textStart, index));
    if (sequence.end !== undefined) tokens.push({ value: value.slice(index, sequence.end), width: 0 });
    index = sequence.end ?? sequence.incompleteIntroducerEnd;
    textStart = index;
  }
  if (textStart < value.length) pushText(value.slice(textStart));
  return tokens;
}
...
export function takeTerminalColumns(value: string, count: number): [string, string] {
 ...
 */
export function wrapPlainLine(line: string, width = terminalLineWidth): string[] {
  const safeWidth = Math.max(2, Math.floor(width));
  const safeLine = terminalTokens(line).map(({ value }) => value).join("");
  if (visibleTextWidth(safeLine) <= safeWidth || !safeLine) return [safeLine];
  ...
  for (let word of words) {
    while (word) {
      ...
      if (current.trim()) { flush(); continue; }
...
export function wrapPlainLines(lines: readonly string[], width = terminalLineWidth): string[] {
  return lines.flatMap((line) => wrapPlainLine(line, width));
}

export function wrapPlainText(text: string, width = terminalLineWidth): string {
