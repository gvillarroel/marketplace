...
interface ParsedPlayerFile {
  order: number;
  definition: PlayerDefinition;
}

function parseJsonField(frontmatter: Map<string, string>, key: string): unknown {
...
function parsePlayerFile(path: string, trustedSkills: readonly GithubSkill[]): ParsedPlayerFile {
  ...
  if (!source.startsWith("---\n") || source.includes("\0")) throw new Error(`fixed player requires first-line frontmatter: ${path}`);
  const end = source.indexOf("\n---\n", 4);
  if (end < 0 || end > 4_096) throw new Error(`fixed player has invalid frontmatter: ${path}`);
  const frontmatter = new Map<string, string>();
  for (const line of source.slice(4, end).split("\n")) {
    const match = /^([a-z][a-z-]*):\s*(.+)$/u.exec(line);
    if (!match || !allowedKeys.has(match[1]) || frontmatter.has(match[1])) throw new Error(`fixed player has invalid or duplicate frontmatter: ${path}`);
  ...
  if (!Array.isArray(tools) || tools.length > allowedTools.size || tools.some((tool) => typeof tool !== "string" || !allowedTools.has(tool as HarborTool)) || new Set(tools).size !== tools.length) {
...
export function loadFixedPlayers(directory: URL, trustedSkills: readonly GithubSkill[]): ReadonlyMap<string, PlayerDefinition> {
  ...
  if (!filenames.length || filenames.length > maxFiles) throw new Error(`fixed player directory must contain 1..${maxFiles} Markdown files`);
  ...
  const players = new Map<string, PlayerDefinition>();
  const orders = new Set<number>();
  for (const { order, definition } of parsed) {
    if (players.has(definition.name)) throw new Error(`duplicate fixed player: ${definition.name}`);
    if (orders.has(order)) throw new Error(`duplicate fixed player order: ${order}`);
    orders.add(order);
