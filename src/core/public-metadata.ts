...
const commonRelativeRoots =
  "app|apps|build|config|configs|dist|docs?|home|lib|packages?|plugins?|scripts?|src|test|test-ts|tests?|tmp|users|var|workspace";

const commonFileExtensions =
  "cjs|conf|css|csv|env|go|html|ini|java|js|json|jsx|md|mjs|php|ps1|py|rb|rs|sh|sql|toml|ts|tsx|txt|xml|yaml|yml";
...
interface BoundedPublicInput {
  readonly text: string;
  readonly truncated: boolean;
}

function boundedPublicInput(value: string): BoundedPublicInput {
  const points: string[] = [];
  let truncated = false;
...
function normalizeBoundedPublicMetadata(
  input: BoundedPublicInput,
...
const namedCredentialNames =
  "(?:[A-Za-z][A-Za-z0-9]*_)+(?:TOKEN|SECRET|KEY(?:_ID)?|PASSWORD)|" +
  "(?:DATABASE|DB|REDIS|MONGO(?:DB)?|POSTGRES(?:QL)?|MYSQL)_URL|" +
  "PASSWORD|PASSWD|PWD|SECRET|TOKEN|API[_-]?KEY|ACCESS[_-]?KEY(?:_ID)?";

const doubleQuotedCredential = new RegExp(
  `\\b(${namedCredentialNames})\\b\\s*[:=]\\s*"(?:\\\\[^\\r\\n]|[^"\\r\\n])*(?:"|$)`,
  "gimu",
);
const singleQuotedCredential = new RegExp(
  `\\b(${namedCredentialNames})\\b\\s*[:=]\\s*'(?:\\\\[^\\r\\n]|[^'\\r\\n])*(?:'|$)`,
  "gimu",
);
const unquotedCredential = new RegExp(
  `\\b(${namedCredentialNames})\\b\\s*[:=]\\s*[^\\s,;]+`,
  "giu",
);
 ...
 */
export function redactPublicMetadata(value: string): string {
  return value
    ...
    .replace(
      /-----BEGIN (?:[A-Z0-9]+ ){0,8}PRIVATE KEY-----[\s\S]*?(?:-----END (?:[A-Z0-9]+ ){0,8}PRIVATE KEY-----|$)/gu,
    ...
    .replace(doubleQuotedCredential, (_match, name: string) => `${name}=[redacted]`)
    .replace(singleQuotedCredential, (_match, name: string) => `${name}=[redacted]`)
    .replace(unquotedCredential, (_match, name: string) => `${name}=[redacted]`)
    ...
    .replace(
      /(^|[\s"'`(])\/(?:[\p{L}\p{N}\p{M}._~-]+\/)*[\p{L}\p{N}\p{M}._~-]+(?=$|[\s"'`,.;:!?)}\]])/gu,
      "$1[path]",
    )
    .replace(/(^|[\s"'`(])~[\\/](?:[^\s"'`\\/()]+[\\/])*[^\s"'`()]*/gu, "$1[path]")
    ...
    .replace(
      new RegExp(
        `(^|[\\s"'\\x60(])(?:${commonRelativeRoots})[\\\\/](?:[\\p{L}\\p{N}\\p{M}_.-]+[\\\\/])*[\\p{L}\\p{N}\\p{M}_.-]+(?=$|[\\s"'\\x60,.;:!?)}\\]])`,
        "giu",
      ),
      "$1[path]",
    )
    .replace(
      new RegExp(
        `(^|[\\s"'\\x60(])(?:[\\p{L}\\p{N}\\p{M}_.-]+[\\\\/])+(?:[\\p{L}\\p{N}\\p{M}_.-]+\\.)+(?:${commonFileExtensions})(?=$|[\\s"'\\x60,.;:!?)}\\]])`,
        "giu",
      ),
      "$1[path]",
    );
...
export function publicMetadataText(value: string, maximumCodePoints = 500): string | undefined {
...
export function publicErrorText(
  ...
  if (commandsToProtect.length > 32) throw new Error("too many public slash commands to protect");
  const commandPattern = new RegExp(
    `/(?:${commandsToProtect.join("|")})(?=$|[\\s,.;:!?)}\\]])`,
    "gu",
  );
  const commands: Array<{ readonly marker: string; readonly command: string }> = [];
  const occupiedMarkers = new Set(input.text);
  let nextMarker = 0xe000;
  const protectedValue = input.text.replace(
    commandPattern,
    (command) => {
      while (occupiedMarkers.has(String.fromCodePoint(nextMarker))) nextMarker += 1;
      if (nextMarker > 0xf8ff) throw new Error("public command marker space exhausted");
  ...
  let result = normalizeBoundedPublicMetadata(
    { text: protectedValue, truncated: input.truncated },
    maximumCodePoints,
  );
...
export function publicTaskLabel(task: string, maximumCodePoints = 72): string {
