...
function contained(parent: string, child: string): string {
  ...
  if (!rel || rel.startsWith("..") || isAbsolute(rel)) throw new Error("unsafe OpenCode title-key location");
  return target;
}

function keyPaths(home: string): { readonly directory: string; readonly key: string } {
...
async function requireSafeDirectory(directory: string, create: boolean): Promise<boolean> {
  try {
    const stat = await lstat(directory);
    if (stat.isSymbolicLink() || !stat.isDirectory()) throw new Error("unsafe OpenCode title-key directory");
    return true;
  } catch (error: any) {
...
async function readValidatedKey(home: string): Promise<Buffer | undefined> {
  const paths = keyPaths(home);
  if (!await requireSafeDirectory(paths.directory, false)) return undefined;
  let before;
  try { before = await lstat(paths.key); }
...
async function loadOrCreateKey(home: string): Promise<Buffer> {
  ...
  const temporary = contained(paths.directory, join(
    paths.directory,
    `${keyFileName}.tmp-${process.pid}-${randomBytes(12).toString("base64url")}`,
  ));
  let handle;
  ...
  try {
    ...
    try {
      ...
      if (!published || !published.equals(generated)) throw new Error("published OpenCode title key could not be verified");
      return published;
    } catch (error: any) {
...
function signedBody(
  project: string,
  invocation: OpenCodeHarborInvocation,
  agent: string,
  nonce: string,
  sessionID: string,
): string {
...
export async function prepareSignedOpenCodeHarborTitle(
  home: string,
  project: string,
  invocation: OpenCodeHarborInvocation,
  agent: string,
): Promise<OpenCodeHarborTitleSigner> {
  if (!isHarborId(agent)) throw new Error("invalid OpenCode Harbor title agent");
  const key = await loadOrCreateKey(home);
  const nonce = randomBytes(nonceBytes).toString("base64url");
  return (sessionID: string): string => {
    if (!sessionID || sessionID.length > 512) throw new Error("invalid OpenCode Harbor session ID");
...
export async function verifySignedOpenCodeHarborTitle(
  home: string,
  project: string,
  title: string,
  sessionID: string,
): Promise<OpenCodeHarborTitleClaim | undefined> {
...
function verifyWithKey(
  ...
  title: string,
  sessionID: string,
): OpenCodeHarborTitleClaim | undefined {
  const parsed = signedTitlePattern.exec(title);
  if (!parsed) return undefined;
  const invocation = parsed[1] as OpenCodeHarborInvocation;
  const agent = parsed[2];
  if (!isHarborId(agent)) return undefined;
...
export async function verifySignedOpenCodeHarborTitles(
  home: string,
  project: string,
  sessions: readonly { readonly title: string; readonly sessionID: string }[],
...
export async function loadOpenCodeHarborTitleVerifier(
  home: string,
  project: string,
): Promise<OpenCodeHarborTitleVerifier | undefined> {
  const key = await readValidatedKey(home);
...
export function looksLikeOpenCodeHarborTitle(title: string): boolean {
