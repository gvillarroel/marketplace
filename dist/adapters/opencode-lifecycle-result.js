...
function expectedBenchMutation(args) {
...
export function assertOpenCodeLifecycleMutationTruth(command, args, result) {
    if (command === "retire") {
    ...
    if (!expected) {
        if (!args.trim() || args.trim() === "list" || args.trim().startsWith("list "))
            return;
