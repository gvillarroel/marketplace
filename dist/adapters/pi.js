...
function leadRosterRow(definition, busy = false) {
...
function boundedLeadRoster(rows, maximumCharacters = 6_000) {
    const shown = [];
    let length = 0;
...
export default function agentHarbor(pi) {
    ...
    const trackRootExecution = (runId, callerSignal, execute) => {
    ...
    const createDelegateTool = (cwd, leadSessionOptions, parentRunId, delegationRoster) => {
        ...
        const compactRoster = boundedLeadRoster([...delegationRoster].map(([id, definition]) => leadRosterRow(definition, runtime.activeProjectRuns(cwd).some((run) => run.kind !== "contractor" && run.agent === id))));
        return {
            ...
            execute: async (_id, params, signal, _update, context) => {
                ...
                if (!player)
                    throw new Error(`delegation target ${params.agent} is not in this team-lead roster snapshot`);
                const busy = runtime.activeProjectRuns(project).find((run) => run.kind !== "contractor" && run.agent === params.agent && run.rootRunId !== parentRunId);
                if (busy)
                    throw new Error(`delegation target ${params.agent} is busy in ${busy.rootRunId}; wait or stop that run`);
                if (calls >= 6)
    ...
    const createTeamRosterTool = (cwd, rosterSnapshot) => ({
        ...
        execute: async (_id, params, _signal, _update, context) => {
            if (typeof params.query !== "string" || [...params.query].length > 80)
            ...
            const text = matches.length
                ? [`Lead-start roster snapshot · showing ${shown.length}/${matches.length}`,
                    ...shown.map((definition) => leadRosterRow(definition, runtime.activeProjectRuns(project).some((run) => run.kind !== "contractor" && run.agent === definition.name))),
    ...
    const createScoutTools = (cwd, onJoinCommitted) => {
        let filterCalls = 0;
        let joinCalls = 0;
        return [{
                ...
                execute: async (_id, params, signal) => {
                    if (filterCalls >= 3)
                        throw new Error("talent scout filter limit reached (3)");
                    filterCalls += 1;
                ...
                execute: async (_id, params, signal, _update, context) => {
                    if (joinCalls >= 1)
                        throw new Error("talent scout may join at most one player per run");
                    joinCalls += 1;
                    const project = context?.cwd || cwd;
                    const text = await runDeterministicCommand("pi", "join", params.definition, project, signal);
                    const joined = JSON.parse(params.definition);
    ...
    const startPlayer = (player, task, cwd, model, thinkingLevel, signal, onScoutJoinCommitted = () => { }) => {
        if (!task.trim())
        ...
        try {
            ...
            return {
                runId,
                result: trackRootExecution(runId, signal, (effectiveSignal) => executeRun({ ...player, task }, runId, cwd, sessionOptions, effectiveSignal, additionalTools, customTools)),
            };
        }
        catch (error) {
    ...
    pi.registerCommand("scout", {
        ...
        handler: async (args, ctx) => {
            ...
            let refresh = "";
            try {
                run = startPlayer(scoutPlayer, args, ctx.cwd, ctx.model, pi.getThinkingLevel(), ctx.signal, (id) => { joinedPlayer = id; });
