...
import { assertHarborCustomToolAccess, formatHarborTeamRosterSnapshot, harborCustomToolNames, harborCustomToolPolicy, harborStaticCustomToolSpecs, HarborScoutTurnGuard, maximumHarborTeamRosterMembers, validateHarborCustomToolArguments, } from "../core/custom-tools.js";
...
import { formatPiMissionReport, formatPiProjectLiveStatus, formatPiProjectLiveWidget, PiTeamRuntime, settlePiRootPromises, } from "./pi-team-runtime.js";
import { collectPiTeamMembers, formatPiTeamView } from "./pi-team-view.js";
import { claimValidatedSharedAgentActivity, readSharedAgentActivities, withSharedRosterMutationGate, } from "./opencode-agent-activity.js";
...
function boundedPiModelPart(value) {
    if (typeof value !== "string" || value.length > 200)
        return undefined;
    const normalized = value.trim();
    return normalized || undefined;
}
function isPiOfflinePlaceholder(model) {
    if (model === undefined)
        return false;
    const provider = boundedPiModelPart(model.provider)?.toLowerCase();
    const id = boundedPiModelPart(model.id ?? model.model)?.toLowerCase();
    const api = boundedPiModelPart(model.api)?.toLowerCase();
    return provider === "unknown" && (id === "unknown" || id === "default") &&
        (api === undefined || api === "unknown") && model.maxTokens === 0;
}
function resolveInheritedPiModel(ctx) {
    ...
    if (current === undefined || isPiOfflinePlaceholder(current)) {
        try {
            ...
            if (error instanceof Error && /^Pi (?:has|reports) /u.test(error.message))
                throw error;
        }
    ...
    if (typeof registry?.getAvailable === "function" && typeof registry.getError === "function") {
        try {
            ...
            if (error instanceof Error && /^selected Pi model /u.test(error.message))
                throw error;
        }
    }
    return current;
}
function resolveConfiguredPiModel(configured, ctx) {
    if (configured === undefined)
        return resolveInheritedPiModel(ctx);
    ...
    if (!provider || !id)
        throw new Error(`configured Pi model must use provider/model syntax: ${configured}`);
    const resolved = ctx.modelRegistry?.find(provider, id);
    if (!resolved)
        throw new Error(`configured Pi model is unavailable: ${provider}/${id}`);
...
function configuredPiProvider(route) {
    ...
    if (!provider || !id)
        throw new Error(`configured Pi model must use provider/model syntax: ${route}`);
    return provider;
}
async function capturePiProviderProjections(ctx, requiredProviderIds) {
    const registry = ctx.modelRegistry;
    const projections = [];
    const seen = new Set();
    for (const rawProviderId of requiredProviderIds) {
        ...
        try {
            ...
            throw new Error(`Pi could not inspect the registered provider configuration for ${providerId}`);
        }
        let runtimeKey;
        let authSource;
        try {
            authSource = registry?.getProviderAuthStatus?.(providerId)?.source;
        }
        catch {
            throw new Error(`Pi could not inspect authentication for provider ${providerId}`);
        }
        if (authSource === "runtime") {
            ...
            try {
                runtimeKey = await registry.getApiKeyForProvider(providerId);
            }
            catch {
                throw new Error(`Pi could not transfer runtime-only authentication for provider ${providerId}`);
            }
            if (!runtimeKey)
                throw new Error(`Pi runtime-only authentication is unavailable for provider ${providerId}`);
        }
        if (config === undefined && runtimeKey === undefined)
            continue;
        projections.push({
            id: providerId,
...
function playerKind(player) {
    if (player.name === "team-lead")
        return "manager";
    if (player.name === scoutPlayer.name)
        return "utility";
    if (rolePlayers.has(player.name))
        return "fixed";
    if (bundledPlayers.has(player.name))
        return "bundled";
    return "personal";
}
function safeUi(action) {
    ...
    catch { /* A presentation failure must not change command semantics. */ }
}
function notify(ctx, message, level) {
    ctx.ui.notify(message, level);
}
function failCommand(ctx, message) {
...
function publicPiToolFailure(error) {
...
function zeroModelResult(command, result) {
    return wrapPlainText(`Agent Harbor /${command} · 0 model tokens\n${result}`);
}
function boundedPiTeamOutput(text, mandatoryTail = []) {
    ...
    const omission = (count) => `+${count} /team lines omitted; narrow with a field filter or /team run:<id>.`;
...
function stopResult(result) {
    return boundedPiTeamOutput(`Agent Harbor stop · 0 model tokens\n${result}`);
}
function requireBoundedArguments(value, maximumBytes, label) {
...
function piTeamHelp(value) {
    const match = /^(?:help|--help)(?:\s+page:([1-9]\d*))?$/u.exec(value);
    if (!match)
        throw new Error("usage: /team help [page:1|page:2|page:3]");
    ...
    if (page > pages.length)
        throw new Error(`team help page ${page} is out of range; available pages: 1-${pages.length}`);
    return wrapPlainText(pages[page - 1].join("\n"));
}
function humanError(command, error, deterministic = false) {
    const raw = error instanceof Error ? error.message : String(error);
...
function modelPreflightError(command, error) {
    const message = humanError(command, error);
    return message.includes(preflightZeroLine) ? message : `${message}\n${preflightZeroLine}`;
}
function withoutViewHeader(view) {
    return view.split(/\r?\n/gu).slice(1).join("\n");
}
function conciseLifecycleResult(command, args, raw, lifecycle) {
    if (command === "join") {
...
export function requirePiJoinLifecycleOutcome(args, lifecycle) {
    const input = JSON.parse(args);
...
export function requirePiRetireLifecycleOutcome(args, lifecycle) {
...
function expectedPiBenchMutation(args) {
    const match = /^(on|off)\s+(.+)$/u.exec(args.trim());
    if (!match)
        throw new Error("Agent Harbor bench mutation could not be verified");
...
export function requirePiBenchLifecycleOutcome(args, lifecycle) {
...
function conciseBenchLifecycleResult(lifecycle) {
    ...
    if (lifecycle.status === "already-current")
        rows.push("No roster files changed.");
    return rows.join("\n");
}
...
function sameProject(left, right) {
    try {
        return sameCanonicalProject(left, right);
    }
    catch {
        return false;
    }
}
function childToolProject(context, expected) {
...
function leadRosterPreviewRow(definition, busy = false) {
    const capacity = [
        ...definition.tools,
...
function boundedLeadRoster(rows, maximumCharacters = 1_500) {
    const shown = [];
    let length = 0;
    for (const row of rows) {
        const increment = row.length + (shown.length ? 2 : 0);
        if (length + increment > maximumCharacters)
            break;
        shown.push(row);
        length += increment;
    }
...
export default function agentHarbor(pi) {
    ...
    const withRosterLifecycleGate = async (action) => {
    ...
    const rootAbortControllers = new Map();
    const rootPromises = new Map();
    const rootSettlements = new Map();
    const rootSettlementProjects = new Map();
    const lateSettlementNotifications = new Set();
    ...
    const failSharedActivityOwnership = (runId, detail) => {
        const run = runtime.get(runId);
        const prior = sharedActivityAuthorityFailures.get(runId);
        const owner = run ? `${run.agent}'s` : "the player's";
        const failure = prior ?? new Error(`Agent Harbor lost ${owner} exact project-shared activity ownership ${detail}; model work was aborted`);
        sharedActivityAuthorityFailures.set(runId, failure);
        if (run && run.state !== "cleanup-error")
    ...
    runtime.subscribe((runId) => {
        ...
        if (!phase || claim.setPhase(phase))
            return;
        failSharedActivityOwnership(runId, `before ${phase}`);
    });
    const sharedRunIdentity = (harness, agent) => `${harness}:${process.pid}:${agent}:${randomBytes(12).toString("base64url")}`;
    const releaseSharedActivity = (runId) => {
        const claim = sharedActivityClaims.get(runId);
        if (!claim)
            return true;
    ...
    const releaseSharedActivityAfter = (runId, promise) => promise.then((value) => {
    ...
    const currentPersistentPlayer = (project, id) => {
        if (id === scoutPlayer.name)
            return scoutPlayer;
        const fixed = rolePlayers.get(id);
        return fixed ?? loadPiActivePlayer(project, id);
    };
    const validatePersistentAdmission = (project, expected, expectedRoster) => {
        ...
        const { task: _invocationTask, ...expectedProfile } = expected;
        if (playerDefinitionDigest(current) !== playerDefinitionDigest(expectedProfile)) {
    ...
    const persistentBusyAgents = (project) => new Set([
        ...runtime.activeProjectRuns(project)
            .filter(({ kind }) => kind !== "contractor")
            .map(({ agent }) => agent),
        ...readSharedAgentActivities(project).map(({ agent }) => agent),
    ]);
    const formattedPlayerRosterSnapshot = (project, rosterSnapshot, query = "") => {
        const busyAgents = persistentBusyAgents(project);
        return formatHarborTeamRosterSnapshot([...rosterSnapshot.values()].map((definition) => ({
            id: definition.name,
            ...
            ...(definition.model
                ...
                : {}),
            availability: busyAgents.has(definition.name) ? "busy" : "ready",
        })), query);
    };
    const beginClaimedPersistentRun = (project, definition, kind, input, claimKind, expectedRoster) => {
        const claim = claimValidatedSharedAgentActivity(project, definition.name, claimKind, sharedRunIdentity("pi", definition.name), "pi", () => validatePersistentAdmission(project, definition, expectedRoster));
        try {
    ...
    const invalidateCompletionRoster = (project) => {
    ...
    const completionMembers = async (project) => {
    ...
    const rootToolSignal = (rootRunId, toolSignal) => {
        ...
        if (!rootSignal)
            throw new Error("Agent Harbor root execution authority is no longer available");
        const signal = combineSignals(rootSignal, toolSignal);
        signal.throwIfAborted();
        return signal;
    };
    const trackRootExecution = (runId, project, callerSignal, execute) => {
        const controller = rootAbortControllers.get(runId) ?? new AbortController();
        rootAbortControllers.set(runId, controller);
        const authorityFailure = sharedActivityAuthorityFailures.get(runId);
        if (authorityFailure && !controller.signal.aborted)
        ...
        const aborted = new Promise((_resolve, reject) => {
            const fail = () => reject(effectiveSignal.reason instanceof Error
        ...
        const settlement = releaseSharedActivityAfter(runId, execution.catch((error) => {
            ...
            if (rootAbortControllers.get(runId) === controller)
        ...
        tracked = Promise.race([settlement, aborted]).finally(() => {
            ...
            if (rootPromises.get(runId) === tracked)
                rootPromises.delete(runId);
        });
        rootPromises.set(runId, tracked);
        return tracked;
    };
    const stopProjectRoots = (project, selector = "all") => {
    ...
    const stopProjectRootsMessage = (result, empty = "No project-shared persistent-player work is visible; disposable contractor work is process-local.") => {
        const localLines = [
    ...
    const assertRootStartAllowed = (project, agent, kind) => {
        ...
        if (busy)
            throw new Error(`${agent} is already working in ${busy.rootRunId}; wait or use /team stop ${busy.rootRunId}`);
    };
    const captureSessionOptions = async (ctx, model, thinkingLevel, additionalProviderIds = []) => {
        const providerProjections = await capturePiProviderProjections(ctx, [model.provider, ...additionalProviderIds]);
        return {
            model,
            thinkingLevel,
    ...
    const rosterSnapshotDigest = (snapshot) => [...snapshot.entries()]
        .map(([id, definition]) => `${id}:${playerDefinitionDigest(definition)}`)
        .sort()
        .join("\n");
    const destructiveRosterTargets = (command, args) => {
        const value = args.trim();
        if (command === "retire")
            return value ? [value] : [];
        if (command === "join") {
    ...
    const assertRosterMutationAllowed = (project, command, args, ignoredRunId) => {
        for (const target of destructiveRosterTargets(command, args)) {
            ...
            const busy = runtime.activeProjectRuns(project).find((run) => run.kind !== "contractor" && run.agent === target && run.id !== ignoredRunId);
            if (busy) {
                throw new Error(`cannot ${action} while it is ${busy.state} in ${busy.rootRunId}; ` +
                    `use /team stop ${busy.rootRunId}, then wait for cleanup to settle`);
            }
            const owner = runtime.activeProjectRuns(project).find((run) => run.parentRunId === undefined && run.id !== ignoredRunId && rootRosterReservations.get(run.rootRunId)?.has(target));
            if (owner) {
                throw new Error(`cannot ${action} while ${owner.agent} owns its active roster snapshot in ${owner.rootRunId}; ` +
                    `use /team stop ${owner.rootRunId}, then wait for cleanup to settle`);
            }
        }
    };
    const withProjectRosterMutationGate = (project, command, args, action, ignoredRunId) => {
        ...
        return withSharedRosterMutationGate(project, targets, label, action, ignoredRunId === undefined ? undefined : sharedActivityClaims.get(ignoredRunId)?.snapshot.claimToken);
    };
    const teamViewRuntimeOptions = (ctx) => {
        let nextThinking;
        try {
            nextThinking = pi.getThinkingLevel();
        }
        catch { /* Keep deterministic inspection available. */ }
        const currentModel = ctx.model;
        const rawProvider = boundedPiModelPart(currentModel?.provider);
        const rawId = boundedPiModelPart(currentModel?.id ?? currentModel?.model);
        const noActiveModel = currentModel === undefined || isPiOfflinePlaceholder(currentModel);
        let registryReportsNoAvailableModels = false;
        let availableModelCount;
    ...
    const createOrchestrator = (cwd, sessionOptions, additionalTools = [], customTools = [], observer) => new PiOrchestrator(cwd, loadHostSdk, additionalTools, undefined, customTools, undefined, sessionOptions, observer);
    const executeRun = (definition, runId, cwd, sessionOptions, signal, additionalTools = [], customTools = []) => (async () => {
        try {
            const result = await createOrchestrator(cwd, sessionOptions, additionalTools, customTools, runtime.observer(runId)).run(definition, signal);
    ...
    const startDefinition = (definition, cwd, sessionOptions, kind, signal, parentRunId, additionalTools = [], customTools = []) => {
        completionProject = cwd;
        requireBoundedArguments(definition.task, maximumPiTaskBytes, "contract task");
        if (parentRunId === undefined)
            assertRootStartAllowed(cwd, definition.name, kind);
        const runInput = {
            project: cwd,
            ...
            ...(sessionOptions.model === undefined ? {} : {
                modelSource: definition.model === undefined ? "inherited" : "configured",
            }),
        ...
        const execute = (effectiveSignal) => executeRun(definition, runId, cwd, sessionOptions, effectiveSignal, additionalTools, customTools);
        const result = parentRunId === undefined
            ? trackRootExecution(runId, cwd, signal, execute)
            : releaseSharedActivityAfter(runId, execute(combineSignals(signal, rootAbortControllers.get(runtime.get(runId).rootRunId)?.signal)));
        return {
            runId,
            result,
        };
    };
    const createDelegateTool = (cwd, leadSessionOptions, parentRunId, delegationRoster) => {
        ...
        const compactRoster = boundedLeadRoster([...delegationRoster].map(([id, definition]) => leadRosterPreviewRow(definition, busyAtAdmission.has(id))));
        const spec = harborStaticCustomToolSpecs[harborCustomToolNames.delegate];
        const policy = harborCustomToolPolicy(spec.name);
        const staticParameters = spec.parameters;
        const parameters = {
            ...spec.parameters,
            properties: {
            ...
            execute: async (_id, params, signal, _update, context) => {
                try {
                    ...
                    if (call.kind !== "delegate")
                        throw new Error("invalid Agent Harbor delegate dispatch");
                    const player = delegationRoster.get(call.agent);
                    if (!player)
                        throw new Error(`delegation target ${call.agent} is not in this team-lead roster snapshot`);
                    const busy = runtime.activeProjectRuns(project).find((run) => run.kind !== "contractor" && run.agent === call.agent && run.rootRunId !== parentRunId);
                    if (busy)
                        throw new Error(`delegation target ${call.agent} is busy in ${busy.rootRunId}; wait or stop that run`);
                    if (calls >= policy.maximumCalls)
                        throw new Error(`delegation limit reached (${policy.maximumCalls})`);
                    if (delegatedAgents.has(call.agent))
                        throw new Error(`already delegated to ${call.agent} in this team-lead run`);
    ...
    const createTeamRosterTool = (cwd, rosterSnapshot, parentRunId) => {
            ...
            execute: async (_id, params, signal, _update, context) => {
                try {
                    ...
                    if (call.kind !== "team-roster")
                        throw new Error("invalid Agent Harbor team-roster dispatch");
                    if (calls >= policy.maximumCalls)
                        throw new Error(`team roster limit reached (${policy.maximumCalls})`);
                    calls += 1;
    ...
    const createScoutTools = (cwd, onJoinCommitted, rosterSnapshot, parentRunId) => {
        ...
        return [{
                ...
                execute: async (_id, params, signal, _update, context) => {
                    ...
                    try {
                        ...
                        if (call.kind !== "filter-skills")
                            throw new Error("invalid Agent Harbor skill-filter dispatch");
                ...
                execute: async (_id, params, signal, _update, context) => {
                    ...
                    try {
                        ...
                        if (call.kind !== "join-player")
                            throw new Error("invalid Agent Harbor join-player dispatch");
                        const committed = await withRosterLifecycleGate(() => withProjectRosterMutationGate(project, "join", call.definition, async () => {
                            const result = await runDeterministicCommandResult("pi", "join", call.definition, project, effectiveSignal);
                            const joined = JSON.parse(call.definition);
                        ...
                        guard.succeed(ticket);
                        return {
                            content: [{ type: "text", text: conciseLifecycleResult("join", call.definition, committed.text, committed.lifecycle) }],
                            details: {
                                harness: "pi",
                                action: "join",
    ...
    const preparePlayerRoster = (player, cwd) => {
    ...
    const assertPlayerRosterStartPreflight = (player, cwd, rosterSnapshot) => {
        ...
        if (player.name === "team-lead") {
            ...
            if (sharedClaims > maximumExistingClaims) {
                throw new Error(`team-lead needs two project-shared slots for its root and first specialist; ` +
                    `${sharedClaims}/${maximumConcurrentPiRoots} are already occupied. Wait or use /team stop all`);
            }
        }
    };
    const startPlayer = (player, task, cwd, sessionOptions, rosterSnapshot, signal, onScoutJoinCommitted = () => { }) => {
        ...
        const runInput = {
            ...
            ...(model === undefined ? {} : {
                modelSource: player.model === undefined ? "inherited" : "configured",
            }),
            ...(thinkingLevel === undefined ? {} : { thinking: thinkingLevel }),
        };
        const runId = beginClaimedPersistentRun(cwd, player, playerKind(player), runInput, "direct", rosterSnapshot.size ? rosterSnapshot : undefined);
        if (rosterSnapshot.size) {
        ...
        try {
            ...
            return {
                runId,
                result: trackRootExecution(runId, cwd, signal, (effectiveSignal) => executeRun({ ...player, task }, runId, cwd, sessionOptions, effectiveSignal, additionalTools, customTools)),
            };
        }
        catch (error) {
            rootRosterReservations.delete(runId);
    ...
    const startPlayerAfterPreflight = async (player, managed, task, cwd, sessionOptions, rosterSnapshot, signal, onScoutJoinCommitted = () => { }) => withRosterLifecycleGate(() => {
        signal?.throwIfAborted();
        let current = player;
        if (managed) {
            try {
                current = loadPiActivePlayer(cwd, player.name);
            }
            catch {
    ...
    const disposeLiveUi = (surface) => {
        ...
        if (liveUiSurface === surface)
            liveUiSurface = undefined;
    };
    const renderLiveUi = (surface) => {
        if (liveUiSurface !== surface)
            return;
        ...
        safeUi(() => surface.ctx.ui.setStatus?.(liveUiKey, formatPiProjectLiveStatus(runtime, surface.project)));
        safeUi(() => surface.ctx.ui.setWidget?.(liveUiKey, formatPiProjectLiveWidget(runtime, surface.project), { placement: "aboveEditor" }));
    };
    const focusLiveUi = (ctx) => {
        ...
        if (liveUiSurface)
            disposeLiveUi(liveUiSurface);
        const surface = { project: ctx.cwd, ctx };
    ...
    const notifyLateSettlement = (ctx, run) => {
        if (lateSettlementNotifications.has(run.runId))
            return;
        ...
        void settlement.then(() => undefined, () => undefined).then(() => {
            ...
            setTimeout(() => {
                ...
                if (!state || state === "starting" || state === "working" || state === "cleaning")
                    return;
                const level = state === "completed" ? "info"
    ...
    const registerPlayer = (id, fixed, display = fixed, refresh = false) => {
        if (registered.has(id) && !refresh)
            return;
        ...
        pi.registerCommand(id, {
            ...
            handler: async (args, ctx) => {
                ...
                try {
                    ...
                    let player;
                    try {
                        player = fixed ?? loadPiActivePlayer(ctx.cwd, id);
                    }
                    catch {
                        throw new Error(`active managed player preflight failed: ${id}`);
                    }
                    if (!args.trim())
                        throw new Error(`/${player.name} requires a non-empty task`);
                    ...
                    const additionalProviderIds = player.name === "team-lead"
                        ? [...rosterSnapshot.values()].flatMap(({ model: route }) => route === undefined ? [] : [configuredPiProvider(route)])
                        : [];
                    const sessionOptions = await captureSessionOptions(ctx, model, pi.getThinkingLevel(), additionalProviderIds);
                    run = await startPlayerAfterPreflight(player, fixed === undefined, args, ctx.cwd, sessionOptions, rosterSnapshot, ctx.signal);
    ...
    const syncActivePlayers = (project) => {
    ...
    const teamCompletions = async (prefix) => {
        try {
            ...
            for (const member of members) {
                if (memberItems.length >= maximumPiCompletionItems)
                    break;
            ...
            const observedOwnerItems = [];
            try {
                const values = new Set();
                for (const { ownerRuntime, processID } of readSharedAgentActivities(completionProject)) {
                    if (ownerRuntime === "pi" || ownerRuntime === "copilot")
                        values.add(`owner:${ownerRuntime}`);
    ...
    const benchCompletions = async (prefix) => {
        try {
            const members = await completionMembers(completionProject);
            const normalized = prefix.trim().toLowerCase();
            const items = [];
            const append = (value) => {
                if (items.length >= maximumPiCompletionItems)
                    return;
                if (!normalized || value.startsWith(normalized))
                    items.push({ value, label: value });
            };
            for (const value of ["list", "on all", "off all"])
                append(value);
            for (const member of members) {
                if (items.length >= maximumPiCompletionItems)
                    break;
                if (member.kind !== "bundled" && member.kind !== "personal")
                    continue;
                for (const action of ["list", "on", "off"])
                    append(`${action} ${member.id}`);
            }
        ...
        handler: async (args, ctx) => {
            ...
            try {
                ...
                if (stopValue === "stop" || stopValue.startsWith("stop ")) {
                    ...
                    if (!matched && !result.activityAuthorityUnavailable)
                        throw new Error(`no active Harbor root matches ${selector}`);
                ...
                notify(ctx, boundedPiTeamOutput(result, help && discoveryWarning ? ["", `Warning: ${discoveryWarning}`] : []), "info");
            }
            catch (error) {
                failCommand(ctx, boundedPiTeamOutput(humanError("team", error, true)));
            }
        },
    });
    registered.add("team");
    for (const name of commandNames) {
            ...
            pi.registerCommand(name, {
                ...
                handler: async (args, ctx) => {
                    ...
                    try {
                        ...
                        let { lifecycle, refresh } = await withRosterLifecycleGate(() => withProjectRosterMutationGate(ctx.cwd, name, args, async () => {
                        ...
                        let view;
                        try {
                            view = await formatPiTeamView(ctx.cwd, runtime, { title: "bench", ...teamViewRuntimeOptions(ctx) });
                        }
                        catch {
                            discoveryWarning = metadataRefreshWarning;
                            if (!refresh)
                        ...
                        failCommand(ctx, humanError(name, error, true));
                    }
                },
            });
            registered.add(name);
        ...
        pi.registerCommand(name, {
            ...
            handler: async (args, ctx) => {
                completionProject = ctx.cwd;
                try {
                    requireBoundedArguments(args, name === "join" ? maximumPiDefinitionBytes : maximumPiFilterBytes, `/${name} arguments`);
                    if (name === "retire" && !args.trim())
                        throw new Error("usage: /retire <personal-id>");
                    const execute = async () => {
                        const result = await runDeterministicCommandResult("pi", name, args, ctx.cwd, ctx.signal, name === "list-skills" ? "ansi" : "plain");
                        let refresh = "";
    ...
    pi.registerCommand("scout", {
        ...
        handler: async (args, ctx) => {
            ...
            const reconcileCommittedJoin = (id) => {
                joinedPlayer = id;
                let reconciled = true;
                try {
                    syncActivePlayers(ctx.cwd);
                ...
                if (handlerSettled) {
                    safeUi(() => notify(ctx, wrapPlainText(`Roster commit preserved: ${id} is joined and active in this project. ` +
                        (reconciled
            ...
            try {
                ...
                if (!args.trim())
                    throw new Error(`/${scoutPlayer.name} requires a non-empty task`);
                ...
                const sessionOptions = await captureSessionOptions(ctx, model, pi.getThinkingLevel());
                run = await startPlayerAfterPreflight(scoutPlayer, false, args, ctx.cwd, sessionOptions, rosterSnapshot, ctx.signal, reconcileCommittedJoin);
    ...
    pi.on?.("session_shutdown", async () => {
        for (const controller of rootAbortControllers.values()) {
            if (!controller.signal.aborted)
                controller.abort(new DOMException("Pi session shutdown", "AbortError"));
        }
        try {
    ...
    try {
        ...
        discoveryWarning = "Active alias discovery failed; fixed controls remain available. Inspect /team, then run /reload after repairing the roster.";
    }
}
