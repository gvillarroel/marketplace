...
import { assertHarborCustomToolAccess, formatHarborTeamRosterSnapshot, harborCustomToolNames, harborCustomToolPolicy, harborStaticCustomToolSpecs, HarborScoutTurnGuard, validateHarborCustomToolArguments, } from "../core/custom-tools.js";
...
import { formatPiLiveStatus, formatPiLiveWidget, formatPiMissionReport, PiTeamRuntime, settlePiRootPromises, } from "./pi-team-runtime.js";
...
function conciseLifecycleResult(command, args, raw) {
    if (command === "join") {
...
export default function agentHarbor(pi) {
    ...
    const stopProjectRootsMessage = (result, empty = "No Agent Harbor work is active in this project.") => {
        const lines = [
    ...
    const createDelegateTool = (cwd, leadSessionOptions, parentRunId, delegationRoster) => {
        ...
        const compactRoster = boundedLeadRoster([...delegationRoster].map(([id, definition]) => leadRosterPreviewRow(definition, runtime.activeProjectRuns(cwd).some((run) => run.kind !== "contractor" && run.agent === id))));
        ...
        return {
            ...
            execute: async (_id, params, signal, _update, context) => {
                try {
                    ...
                    calls += 1;
                    const snapshot = formatHarborTeamRosterSnapshot([...rosterSnapshot.values()].map((definition) => ({
                        id: definition.name,
    ...
    const trackUi = async (ctx, run) => {
        ...
        const unsubscribe = runtime.subscribe((changedId) => {
            if (runtime.get(changedId)?.rootRunId === run.runId)
                render();
        });
        render();
    ...
    pi.registerCommand("team", {
        ...
        handler: async (args, ctx) => {
            ...
            try {
                ...
                failCommand(ctx, humanError("team", error, true));
            }
        },
    });
    registered.add("team");
        ...
        pi.registerCommand(name, {
            ...
            handler: async (args, ctx) => {
                ...
                try {
                    ...
                    if (name === "retire") {
                        const id = args.trim();
                        const busy = runtime.activeProjectRuns(ctx.cwd).find((run) => run.kind !== "contractor" && run.agent === id);
                        if (busy) {
                            throw new Error(`cannot retire ${id} while it is ${busy.state} in ${busy.rootRunId}; ` +
                                `use /team stop ${busy.rootRunId}, then wait for cleanup to settle`);
                        }
                    }
                    const result = await runDeterministicCommand("pi", name, args, ctx.cwd, ctx.signal, name === "list-skills" ? "ansi" : "plain");
                    let refresh = "";
    ...
    pi.registerCommand("scout", {
        ...
        handler: async (args, ctx) => {
            ...
            try {
                ...
                const sessionOptions = await captureSessionOptions(ctx, model, pi.getThinkingLevel());
                run = startPlayer(scoutPlayer, args, ctx.cwd, sessionOptions, rosterSnapshot, ctx.signal, reconcileCommittedJoin);
