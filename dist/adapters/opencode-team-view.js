...
import { maximumOpenCodeActiveSessions, maximumOpenCodeMessagesPerSession, maximumOpenCodeMessageSessions, maximumOpenCodeRosterRecords, maximumOpenCodeRosterSnapshotBytes, maximumOpenCodeSessions, openCodePublicIdentifier, openCodePublicLabel, openCodeTaskLabel, } from "./opencode-team-runtime.js";
...
function parseFilter(filter) {
    ...
    if (!value)
        throw new Error(`/team ${rawField}: requires a non-empty value`);
    return { field, value };
}
function parseTeamViewRequest(input) {
    const trimmed = input.trim();
    const section = /^(roster|activity)(?:\s+(\d{1,6}))?$/iu.exec(trimmed);
    if (section) {
        const page = section[2] ? Number(section[2]) : 1;
        if (!Number.isSafeInteger(page) || page < 1)
            throw new Error(`/team ${section[1].toLowerCase()} page must be at least 1`);
        return { section: section[1].toLowerCase(), page, filter: "" };
    }
    const pageTokens = [...trimmed.matchAll(/(?:^|\s)page:(\d{1,6})(?=\s|$)/giu)];
    if (pageTokens.length > 1)
        throw new Error("/team accepts at most one page:<number> selector");
    const page = pageTokens.length ? Number(pageTokens[0][1]) : 1;
    if (!Number.isSafeInteger(page) || page < 1)
        throw new Error("/team page must be at least 1");
...
function memberStatusTerms(member, effectiveStatus, idleAuthoritative) {
    const terms = effectiveStatus === member.availability ? [...availabilityTerms(member.availability)] : [effectiveStatus];
    if (idleAuthoritative && effectiveStatus === "ready")
        terms.push("idle");
    return terms;
}
function memberMatches(member, filter, effectiveStatus = member.availability, idleAuthoritative = false) {
    if (!filter)
        return true;
...
function reservationMatches(reservation, filter) {
    ...
    if (query.field === "run")
        return includes([reservation.id], query.value);
    if (query.field === "owner")
        return equals([reservation.ownerLocator], query.value);
    if (query.field)
        return false;
...
function runMatches(run, filter) {
    ...
    if (query.field === "task")
        return run.taskObserved && includes([run.task], query.value);
    if (query.field === "run")
        return includes([run.id], query.value);
    if (query.field === "mode")
        return equals([run.invocation], query.value);
    if (query.field === "owner")
        return equals([run.ownerLocator], query.value);
    if (query.field)
        return false;
...
function activeTelemetryFilterCompleteness(snapshot, filter) {
    if (!filter)
        return undefined;
...
function quotedPublic(value) {
    return `"${value.replaceAll("\"", "″")}"`;
}
function formattedObservedModel(run) {
    if (!run.model)
        return undefined;
...
function fallbackObservedTotal(usage) {
...
function formatObservedTotal(run) {
...
function availabilitySymbol(member, activeState, activeAuthoritative) {
    if (activeState)
        return activeState === "working" ? "●" : "◐";
...
function visibleMemberState(member, activeState, activeAuthoritative) {
    if (activeState)
        return activeState;
    if (activeAuthoritative)
        return availabilityLabel(member.availability);
    if (member.availability === "ready")
        return "enabled · activity/availability unverified";
    if (member.availability === "reload-required")
        return "enabled · reload required · activity unverified";
    if (member.availability === "bench")
        return "rostered as bench · activity unverified";
    return `${member.availability} · activity unverified`;
}
function renderRoster(members, activity, activeAuthoritative, aliasCollisions) {
...
function renderCompactRoster(members, activity, activeAuthoritative, aliasCollisions) {
...
function compactActivityLines(run) {
...
function compactLeadAccess(members, runs, reservations, activeAuthoritative, directAliasCollisions) {
    if (!activeAuthoritative) {
    ...
    const enabledSpecialists = members.filter((member) => member.kind !== "manager" && member.kind !== "utility" &&
...
function directAliasCollisionNotice(collisions) {
...
function openCodeWorkHelp(collisions) {
    if (!collisions.length) {
...
function currentWarnings(snapshot) {
    const raw = [
...
function diagnosticRepair(reason) {
...
function boundView(lines, clippingNotice = "… view clipped to the 30-line dialog budget; narrow with /team member:<id> or run:<id>.") {
...
export function boundOpenCodeDialogText(value, clippingNotice = "Use the command's page/filter route to inspect omitted content.") {
    return boundView(value.split(/\r?\n/gu), clippingNotice);
}
function boundViewWithFooter(lines, footer, clippingNotice = "… stop details clipped to the 30-line dialog budget; run /team before retrying unresolved work.") {
    ...
    if (bodyLines.length > bodyBudget) {
        ...
        const semanticOmissions = wrapPlainLines(lines.filter((line) => /^\+\d+ (?:roster matches|active entries) hidden;/u.test(line)));
        const reservedTail = [...semanticOmissions, ...noticeLines].slice(0, bodyBudget);
        const nonSemanticBody = wrapPlainLines(lines.filter((line) => !/^\+\d+ (?:roster matches|active entries) hidden;/u.test(line)));
        bodyLines = [
...
export function formatOpenCodeTeamView(snapshot, filterInput = "") {
    ...
    const members = snapshot.members.filter((entry) => memberMatches(entry, filter, activity.get(entry.id) ?? entry.availability, snapshot.activeAuthoritative));
    ...
    const lines = [
        ...
        ...(telemetryFilter?.unknownCount ? [oneTerminalLine(`i ${telemetryFilter.field === "task" ? "Task" : telemetryFilter.field === "model" ? "Model" : "Text"} filter incomplete: ${telemetryFilter.unknownCount} active entr${telemetryFilter.unknownCount === 1 ? "y has" : "ies have"} unobserved ${telemetryFilter.field}; inspect unfiltered /team.`)] : []),
    ...
    return boundViewWithFooter(lines, footer, `Use ${baseRoute}${request.section === "combined" ? ` page:${Math.min(totalPages, request.page + 1)}` : ` ${Math.min(totalPages, request.page + 1)}`} for the next deterministic page.`);
}
/** Renders every current sanitized warning and a recovery action through bounded pages. */
export function formatOpenCodeTeamDiagnostics(snapshot, requestedPage = 1) {
    const warnings = currentWarnings(snapshot);
    ...
    const firstReason = pages.slice(0, pageNumber - 1).reduce((total, entries) => total + entries.filter((line) => /^\d+\. Reason:/u.test(line)).length, 0) + 1;
...
export function formatOpenCodeTeamHelp(directAliasCollisions = [], requestedPage = 1) {
...
export function formatOpenCodeStopResult(result, footer) {
