...
- `copilot-home` is non-empty absolute `COPILOT_HOME`, otherwise the current user's home directory plus `.copilot`. Resolve it with the current environment's home and path facilities; reject a relative override and never assume a path separator or shell family.
- `current-folder` is the process working directory captured at invocation start and canonicalized independently of `copilot-home`. Never derive it from, join it to, or resolve it relative to `copilot-home`, the plugin base, or a literal directory named `current-folder`.
...
- Active profiles live only at `<current-folder>/.github/agents/<id>.agent.md`.
