...
12. `/agent-foundry:join` registers a recurring player on the personal bench and activates it in the current folder. `/agent-foundry:lineup` activates an existing player, `/agent-foundry:leave` returns it to the bench here, `/agent-foundry:retire` removes the personal registration, and `/agent-foundry:contract` forgets its child after the invocation.
13. Reuse a canonical role from `harbor-sdlc-bench` instead of recreating it.
