...
import { materializeSkills, resolveCopilotCliPath, type AgentDefinition } from "./core.js";

export async function runContractor(definition: AgentDefinition, task: string): Promise<string> {
  ...
  try {
    ...
    const session = await client.createSession({
      model,
      ...(model === "auto" ? {} : { reasoningEffort: "low" as const }),
