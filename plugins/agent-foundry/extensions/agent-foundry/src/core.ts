...
export type SkillSource =
  ...
  | { kind: "github"; repo: string; path: string; ref?: string; name?: string };
 ...
 */
export function resolveCopilotCliPath(env: NodeJS.ProcessEnv = process.env): string {
  ...
  throw new Error(
    "Could not locate the GitHub Copilot CLI executable. Set AGENT_HARBOR_CLI_PATH to its absolute path.",
  );
}

const safeName = (value: string) => value.trim().toLowerCase().replace(/[^a-z0-9-]+/g, "-").replace(/^-|-$/g, "");

export function validateDefinition(input: AgentDefinition): AgentDefinition {
  const name = safeName(input.name ?? "");
  if (!name || name.length > 64) throw new Error("Agent name must be 1-64 kebab-case characters.");
...
export function parseDefinition(raw: string): AgentDefinition {
  try { return validateDefinition(JSON.parse(raw)); }
  catch (error) { throw new Error(`Expected a JSON agent definition: ${error instanceof Error ? error.message : String(error)}`); }
}

export function renderAgentMarkdown(input: AgentDefinition): string {
...
export async function savePermanentAgent(input: AgentDefinition, cwd = process.cwd()): Promise<string> {
  const agent = validateDefinition(input);
...
export async function removePermanentAgent(name: string, cwd = process.cwd()): Promise<void> {
  await rm(resolve(cwd, ".github", "agents", `${safeName(name)}.agent.md`), { force: true });
}

export async function materializeSkills(sources: SkillSource[], root?: string): Promise<{ root: string; names: string[]; cleanup: () => Promise<void> }> {
  const target = root ?? await mkdtemp(join(tmpdir(), "agent-harbor-"));
  const names: string[] = [];
  for (const source of sources) {
    const name = safeName(source.name ?? basename(source.path));
    if (!name) throw new Error(`Cannot infer skill name from ${source.path}`);
    const directory = join(target, name);
    await mkdir(directory, { recursive: true });
    let content: string;
    if (source.kind === "local") content = await readFile(resolve(source.path), "utf8");
    else {
      if (!/^[\w.-]+\/[\w.-]+$/.test(source.repo)) throw new Error(`Invalid GitHub repository: ${source.repo}`);
