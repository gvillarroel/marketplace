...
- `agent-foundry`: five roster commands, a user-level bench, six parked SDLC profiles, a team lead, and a trusted GitHub-skill catalog.
...
| Command | Purpose |
| --- | --- |
| `/agent-foundry:bench` | List players or set their current-folder state with `on` and `off`. |
| `/agent-foundry:join` | Register a recurring player at user level and activate it here. |
| `/agent-foundry:retire` | Remove one personal registration and its managed local copy. |
| `/agent-foundry:contract` | Run one synchronous, invocation-scoped subagent without registration. |
| `/agent-foundry:list-skills` | List trusted GitHub skill references; distinct from built-in `/skills`. |

`agents`, `lineup`, and `leave` are intentionally absent: Copilot owns `/agents`, while `lineup` and `leave` duplicated `bench`.
...
```text
/agent-foundry:join {"name":"reviewer","description":"Read-only reviewer","prompt":"Return three evidence-backed findings.","tools":["read","search"],"skills":[]}
```
...
```text
/agent-foundry:join {"name":"zx-maker","description":"Minimal zx author","prompt":"Create the smallest runnable example.","tools":["read","search","edit","execute"],"skills":[{"kind":"github","name":"zx-example-author","repo":"gvillarroel/zx-harness","path":"skills/zx-example-author/SKILL.md","track":"refs/heads/main"}]}
```
...
```text
/agent-foundry:contract {"name":"reviewer","description":"Read-only reviewer","prompt":"Review only; never edit.","tools":["read","search"],"skills":[]} :: review src and return three findings
```
...
The marketplace relies on Copilot's native Markdown agents, skills, commands, and subagent tools. Its policies reduce accidental scope but cannot turn model instructions into an operating-system sandbox.
